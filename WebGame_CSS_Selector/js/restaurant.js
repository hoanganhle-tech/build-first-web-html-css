...
// Edited by Fwy13 for Club IT (Thanks for author flukeout).
/**
 * Edited:
 * Remove type number for level skip
 * Add lang VietNamese
 * Remove share
...
function winGame(){
  $(".table").html('<span class="winner"><strong>Bạn đã làm được 🎇</strong><br>Hãy cố gắng hơn nữa nhé!</span>');
  addNametags();
  finished = true;
  resetTable();
}
...
function loadLevel(){
  ...
  $(".level-header .level-text").html("Cấp độ " + (currentLevel+1) + " trên " + levels.length);
